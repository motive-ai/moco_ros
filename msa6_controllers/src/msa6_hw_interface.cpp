...
MocoHWInterface::MocoHWInterface(ros::NodeHandle &nh, urdf::Model *urdf_model)
    ...
    rosparam_error += !rosparam_shortcuts::get("", nh_, PARAM_NAMESPACE + "/joint_names", joint_names_);
    rosparam_shortcuts::shutdownIfError(PARAM_NAMESPACE, rosparam_error);
}
...
void MocoHWInterface::init() {
    ...
    for (std::size_t joint_id = 0; joint_id < num_joints_; ++joint_id) {
        ROS_DEBUG_STREAM_NAMED(LOGNAME, "Loading joint name: " << joint_names_[joint_id]);
    ...
    ROS_INFO_STREAM_NAMED(LOGNAME, "MoCo Ready.");
}
...
void MocoHWInterface::registerJointLimits(const hardware_interface::JointHandle &joint_handle_position,
    ...
    if (urdf_model_ == NULL) {
        ROS_WARN_STREAM_NAMED(LOGNAME, "No URDF model loaded, unable to get joint limits");
        return;
    }
    ...
    if (urdf_joint == NULL) {
        ROS_ERROR_STREAM_NAMED(LOGNAME, "URDF joint not found " << joint_names_[joint_id]);
        return;
    }
    ...
    if (joint_limits_interface::getJointLimits(urdf_joint, joint_limits)) {
        ...
        ROS_DEBUG_STREAM_NAMED(LOGNAME, "Joint " << joint_names_[joint_id] << " has URDF position limits ["
                                                                << joint_limits.min_position << ", "
                                                                << joint_limits.max_position << "]");
        if (joint_limits.has_velocity_limits) {
          ROS_DEBUG_STREAM_NAMED(LOGNAME, "Joint " << joint_names_[joint_id] << " has URDF velocity limit ["
                                                                  << joint_limits.max_velocity << "]");
        }
    } else {
    if (urdf_joint->type != urdf::Joint::CONTINUOUS)
      ROS_WARN_STREAM_NAMED(LOGNAME, "Joint " << joint_names_[joint_id] << " does not have a URDF "
                            "position limit");
    }
    ...
    if (use_rosparam_joint_limits_) {
        if (joint_limits_interface::getJointLimits(joint_names_[joint_id], nh_, joint_limits)) {
            has_joint_limits = true;
            ROS_DEBUG_STREAM_NAMED(LOGNAME, "Joint " << joint_names_[joint_id]
    ...
    if (use_soft_limits_if_available_) {
        if (joint_limits_interface::getSoftJointLimits(urdf_joint, soft_limits)) {
          ...
          ROS_DEBUG_STREAM_NAMED(LOGNAME, "Joint " << joint_names_[joint_id] << " has soft joint limits.");
        } else {
          ROS_DEBUG_STREAM_NAMED(LOGNAME, "Joint " << joint_names_[joint_id] << " does not have soft joint limits");
        }
    }
    ...
    if (has_soft_limits) {  // Use soft limits
        ROS_DEBUG_STREAM_NAMED(LOGNAME, "Using soft saturation limits");
        ...
        ROS_DEBUG_STREAM_NAMED(LOGNAME, "Using saturation limits (not soft limits)");
...
void MocoHWInterface::loadURDF(ros::NodeHandle &nh, std::string param_name) {
    ...
    while (urdf_string.empty() && ros::ok()) {
        ...
        if (nh.searchParam(param_name, search_param_name)) {
          ROS_INFO_STREAM_NAMED(LOGNAME, "Waiting for model URDF on the ROS param server at location: " <<
    ...
    if (!urdf_model_->initString(urdf_string)) {
        ROS_ERROR_STREAM_NAMED(LOGNAME, "Unable to load URDF model");
    } else {
        ROS_DEBUG_STREAM_NAMED(LOGNAME, "Received URDF from param server");
    }
}
